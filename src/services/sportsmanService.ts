...
export const getAll = async (page = 1, name?: string, gender?: number | null, address?: string, eventType?: number | null, chest_number: string = '', is_archive:boolean = true) => {
  ...
  const { data } = await axiosWithAuth.get(`/sportsman/all?page=${page}${gender ? `&gender=${gender}` : ''}${address ? `&address=${address}` : ''}${eventType ? `&event-type=${eventType}` : ''}${chest_number ? `&chest_number=${chest_number}` : ''}&is_archive=${is_archive}`, {
    params: {
      name,
      limit: 10
    }
